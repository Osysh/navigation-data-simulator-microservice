...
//       // Ensure the last position is exactly the end position
//       if (positions.length === 0 || positions[positions.length - 1].date.getTime() !== endTime) {
//           positions.push({ position: endPos, date: new Date(endTime) });
//       }
...
//         if (i > 0) {
//             let previousDate = flatData[i - 1].date;
//             let currentDate = flatData[i].date;
//             while (currentDate.getTime() - previousDate.getTime() > interval * 100) {
//                 previousDate = new Date(previousDate.getTime() + interval * 100);
//                 tableData.push({ date: previousDate, mobiles: [] }); // Inserting empty position
//             }
//         }
//         tableData.push({ date: flatData[i].date, mobiles: [flatData[i].mobileData] });
//     }
